...
public:
	...
	HRESULT CreateRichEdit(ITextHost *pTextHost, IUnknown **ppUnk)
	{
		...
		if (!m_hmodule) m_hmodule = LoadLibrary(_T("Riched20.dll"));

		if (!m_hmodule) m_hmodule = LoadLibrary(_T("Riched32.dll"));

		if (m_hmodule) TextServicesProc = (PCreateTextServices)GetProcAddress(m_hmodule, "CreateTextServices");
...
BOOL CTxtWinHost::Init(CRichEditUI *re, const CREATESTRUCT *pcs)
{
    ...
    if(FAILED(RichEditHelper.CreateRichEdit(this, &pUnk)))
        goto err;
